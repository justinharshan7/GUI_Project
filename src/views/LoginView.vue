<template>
  ...
  <div class="relative">
    ...
    <span class="relative text-3xl font-black uppercase tracking-tighter text-white">Glitch</span>
        ...
        <!-- Error Message -->
        <div v-if="error" class="bg-red-500/20 border border-red-500/50 text-red-200 text-sm px-4 py-3 rounded-xl mb-4">
          {{ error }}
        </div>
        ...
        <div class="space-y-4">
          <!-- Username -->
          <div>
            <label class="text-white/70 text-xs uppercase tracking-wide font-semibold mb-2 block">Username</label>
            <input
              v-model="username"
              type="text"
              placeholder="Enter your username"
              class="w-full px-4 py-3 rounded-xl bg-white/10 dark:bg-white/5 border border-white/20 text-white placeholder-white/40 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition-all"
            />
          </div>
          ...
          <div>
            <div class="flex items-center justify-between mb-2">
              <label class="text-white/70 text-xs uppercase tracking-wide font-semibold">Password</label>
              <span class="text-emerald-400 text-xs cursor-pointer hover:text-emerald-300">Forgot Password?</span>
            </div>
            <input
              v-model="password"
              type="password"
          ...
          <button
            @click="handleLogin"
            :disabled="loading"
            class="w-full bg-emerald-500 hover:bg-emerald-400 disabled:opacity-50 text-white font-bold py-3.5 rounded-xl transition-all duration-200 hover:shadow-lg hover:shadow-emerald-500/30 mt-2"
          >
        ...
        <p class="text-center text-white/60 text-sm mt-6">
          ...
          <RouterLink to="/register" class="text-emerald-400 font-bold hover:text-emerald-300 transition-colors">
            Create account
          </RouterLink>
        </p>
        ...
        <div class="mt-4 p-3 rounded-xl bg-white/5 border border-white/10">
          <p class="text-white/40 text-xs text-center">Demo: username <span class="text-emerald-400">emilys</span> / password <span class="text-emerald-400">emilyspass</span></p>
        </div>
